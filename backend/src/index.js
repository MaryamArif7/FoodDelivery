...
mongoose
  .connect(
    "mongodb+srv://maryam7:maryam1306@maryam.xoazkgz.mongodb.net/?retryWrites=true&w=majority&appName=maryam",
    {
...
io.on("connection", (socket) => {
  ...
  socket.on("identify", ({ userType, userId }) => {
    ...
    if (userType === "restaurant") {
      ...
      console.log(
        `Driver ${userId} automatically added to 'available-drivers' room`,
      );
  ...
  socket.on("disconnect", () => {
    activeConnections.restaurants.forEach((socketId, restaurantId) => {
      if (socketId === socket.id)
        activeConnections.restaurants.delete(restaurantId);
    });
