...
export default function Driver() {
  ...
  }, [driverId, acceptedOrders]);

  const changeAvailability = async (newStatus) => {
    try {
      setLoading(true);
      const res = await axios.post(
        `http://localhost:5000/api/orders/driver/availability`,
        {
          driverId,
          status: newStatus,
        },
  ...
  const assignDriver = async (order, orderId) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:5000/api/orders/driver/assign`,
        {
          orderId,
          driverId,
        },
  ...
  const updateOrderStatus = async (orderId, newStatus) => {
    ...
    try {
      ...
      const res = await axios.put(
        `http://localhost:5000/api/orders/driver/update/status`,
        {
          orderId,
  ...
  const getStatusBadge = (orderStatus) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-sm font-medium ${statusConfig[orderStatus]}`}
      >
    ...
    <Sidebar>
      <div className="min-h-screen p-6">
        ...
        <div className="mb-6 flex items-center gap-4">
          <h2
            className={`font-semibold ${status === "available" ? "text-green-500" : "text-red-500"}`}
          >
            Status: {status}
          </h2>
          <button
            disabled={loading}
            onClick={() =>
              changeAvailability(status === "available" ? "busy" : "available")
            }
        ...
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          ...
          <p className="text-gray-600 mb-4">
            Accept them if you are able to deliver
          </p>
          {orders.length === 0 ? (
            ...
            <div className="space-y-4">
              {acceptedOrders.map((order) => (
                <div key={order.id} className="border rounded-lg p-4">
                  ...
                  <div className="mt-3">{getStatusButton(order)}</div>
                </div>
              ))}
            </div>
          )}
